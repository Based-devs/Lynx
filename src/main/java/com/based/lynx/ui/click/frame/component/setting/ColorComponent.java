...
public class ColorComponent extends SettingComponent<Color> {
    ...
    public ColorComponent(Setting<Color> setting, float x, float y, float width, float height, ModuleComponent moduleComponent) {
    ...
    public void renderComponent(int mouseX, int mouseY) {
        ...
        if (animation.getAnimationFactor() > 0) {
            ...
            // Picker attributes
            Color color = Color.getHSBColor(hue / 360, 1, 1);
            ...
            bufferbuilder.pos(pickerX + pickerWidth, pickerY, 0).color(color.getRed(), color.getGreen(), color.getBlue(), color.getAlpha()).endVertex();
            ...
            for (SettingComponent<?> settingComponent : this.pickerComponents) {
                if (settingComponent.getSetting() == this.hueSetting && ((SliderComponent) settingComponent).isDragging()) {
                    ...
                    float[] hsb2 = Color.RGBtoHSB(this.finalColor.getRed(), this.finalColor.getGreen(), this.finalColor.getBlue(), null);
                    this.finalColor = new Color(Color.HSBtoRGB(hue / 360, hsb2[1], hsb2[2]));
                }

                // If we are dragging a slider, we don't want to pick a color
            ...
            if (this.draggingPicker) {
                ...
                this.finalColor = new Color(Color.HSBtoRGB(hue / 360, saturation / 100, brightness / 100));
            }

            // Get final HSB colors
            float[] finHSB = Color.RGBtoHSB(this.finalColor.getRed(), this.finalColor.getGreen(), this.finalColor.getBlue(), null);
            ...
            RenderUtil.drawRect(pickerIndicatorX - 1, pickerIndicatorY - 1, 2, 2, this.finalColor.getRGB());
        }

        this.getSetting().setValue(ColorUtil.integrateAlpha(this.finalColor, this.alphaSetting.getValue().floatValue()));
